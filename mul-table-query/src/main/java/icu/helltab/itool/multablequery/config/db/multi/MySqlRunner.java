 ...
 * @since 2016-12-11
 */
public class MySqlRunner implements ISqlRunner {

	private final Log log = LogFactory.getLog(MySqlRunner.class);
	// 单例Query
	public static final MySqlRunner DEFAULT = new MySqlRunner();
	// 默认FACTORY
//    public static SqlSessionFactory FACTORY;
	...
	public MySqlRunner(MybatisSqlSessionFactoryBean sqlSessionFactory) {
		try {
			this.sqlSessionFactory = sqlSessionFactory.getObject();

		} catch (Exception e) {
			log.error("SqlRunner 初始化错误: " + e.getMessage());
		}
	}

	public MySqlRunner(Class<?> clazz) {
		this.clazz = clazz;
	}
	 ...
	 * @return ignore
	 */
	public static MySqlRunner db(Class<?> clazz) {
		return new MySqlRunner(clazz);
	}
	...
	public List<Map<String, Object>> selectLambda(Consumer<SqlLambdaBuilder> consumer) {
		SqlSession sqlSession = sqlSession();
		try {
			String sql = SqlLambdaBuilder.lambda(consumer);
